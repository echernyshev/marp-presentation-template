...
class ThemeResolver {
  ...
  static extractThemeName(cssContent, fallbackFilename = null) {
    // Match /* @theme name */ - supports multi-line comments and Unicode
    // The [\s\S]*? allows matching across newlines (non-greedy)
    // [^\s/*]+ matches any non-whitespace, non-comment-char (supports Unicode)
    const themeDirectiveRegex = /\/\*[\s\S]*?@theme\s+([^\s/*]+)[\s\S]*?\*\//;
