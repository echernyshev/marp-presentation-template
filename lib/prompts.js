...
class Prompts {
  ...
  static _addThemeChoices(themeName, tree, choices, prefix = '', childrenPrefix = '') {
    ...
    // Mark system themes as disabled (built-in to Marp, not copied)
    if (theme.isSystem) {
   ...
   */
  static async promptActiveTheme(selectedThemes, activeTheme = null) {
    ...
    let choices = selectedThemes;
    let defaultTheme = null;

    if (activeTheme && selectedThemes.includes(activeTheme)) {
      defaultTheme = activeTheme;
      choices = selectedThemes.map(theme =>
        theme === activeTheme
          ? { value: theme, name: `${theme} (current)` }
          : theme
      );
    }
    ...
    if (defaultTheme) {
      options.default = defaultTheme;
    }

    return await inquirer.select(options);
  }
